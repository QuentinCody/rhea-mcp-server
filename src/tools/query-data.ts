...
interface QueryEnv {
    RHEA_DATA_DO?: unknown;
}

export function registerQueryData(server: McpServer, env?: QueryEnv): void {
    const handler = createQueryDataHandler("RHEA_DATA_DO", "rhea");

    server.registerTool(
        "rhea_query_data",
        {
            title: "Query Staged Rhea Data",
            description:
                "Query staged Rhea reaction data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
